...
class BarcodeGenerator extends CI_Controller
{
    ...
    {   
        //unlink('assets/barcode/'.$barcode.'.png');
        file_put_contents('assets/barcode/'.$barcode.'.png', $this->getBarcode($barcode, $this::TYPE_CODE_128));
        $order['order'] = $this->dbase_model->selectBy_where_row('*','tbl_order',"ship_no=".$barcode);
        $package['packages'] = $this->dbase_model->displayall('tbl_package_details');
        $data= array_merge($order,$package);
        //echo "<pre>";print_r($order);exit;
        $this->load->view('pages/printlabel.php',$data);
    }
